...
, extend: {
  ...
  , setData: function(o){
	...
	width: function() {
		return this.rbox().width;
	},
	height: function() {
		return this.rbox().height;
	}
  }
...
SVG.RNode = SVG.invent({
		create: function (text) {
			...
			let background = this.rect(content.width() + 10, content.height() + 4).move(content.x() - 5, content.y() - 2).fill("lightgreen");
			background.back();
		},
		inherit: SVG.Nested,
		construct: {
			rnode: function (text) {
				return this.put(new SVG.RNode(text));
			}
		}
	});
